...
class MAACAgent(object):
    ...
    def actor_act(self, epoch, finish_length):
        ...
        if tmp >= self.epsilon and epoch >= 16:  # todo epoch >= 16 经验池已充满？
            ...
            # # new_rewards方式5.4: 平方和
            # new_rewards = [(((new_rewards_average[i] * weight_average) ** 2 - (new_rewards_age[i] * weight_age) ** 2) /
            #                (new_rewards_average[i] * weight_average + new_rewards_age[i] * weight_age) ** 2)
            #                for i in range(len(new_rewards_average))]     # math.log 底数，默认为 e
            ...
            #                range(len(new_rewards_average))]
            # 方式二
            # new_rewards = [(new_rewards_average[i] / self.agent_num * 1.0 - new_rewards_age[i] / epoch * 1.0) for i in
            #                range(len(new_rewards_average))]
            # 方式二 改进
    ...
    def replay(self):
        ...
        for no, agent_memory in self.agent_memory.items():
            ...
            with tf.GradientTape() as tape:  # 根据某个函数的输入变量来计算它的导数,Tensorflow 会把 ‘tf.GradientTape’ 上下文中执行的所有操作都记录在一个磁带上 (“tape”)。 然后基于这个磁带和每次操作产生的导数，用反向微分法（“reverse mode differentiation”）来计算这些被“记录在案”的函数的导数。
                ...
                # TODO：  error 尝试AdamOptimizer  https://www.cnblogs.com/pinard/p/10345762.html
