    ...
    def __init__(self, world, alpha=0.5, beta=0.2, aggregate_reward=False, discrete=True,
        ...
        for agent in self.agents:
            if self.discrete_flag:
                act_space = spaces.Tuple((circle_space.Discrete_Circle(agent.move_r),       # 移动范围      # gym.spaces.Discrete() 创建一个离散的n维空间，n为整数,从space进行抽样
                ...
                obs_space = spaces.Tuple((spaces.MultiDiscrete([self.map_size, self.map_size]),             # 环境的观察
                                          spaces.Box(0, np.inf, [agent.obs_r * 2, agent.obs_r * 2, 2])))    # 边缘设备的状态   # Box空间表示一个n维框  #TODO 三个参数什么意思
                # pos, obs map
